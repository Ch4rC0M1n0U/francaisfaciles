...
class AIService {
  ...
  async generateExercise(request: ExerciseRequest): Promise<GeneratedExercise> {
    ...
    // Analyser les points faibles de l'utilisateur
    ...
    // Déterminer automatiquement la difficulté
  ...
  private async generateWithGemini(prompt: string): Promise<string> {
    ...
    // Nettoyer la réponse en supprimant les délimiteurs markdown
    if (responseText.startsWith('```json\n')) {
      responseText = responseText.substring(8); // Supprimer '```json\n'
    }
    if (responseText.startsWith('```json')) {
      responseText = responseText.substring(7); // Supprimer '```json'
    }
    if (responseText.endsWith('\n```')) {
      responseText = responseText.substring(0, responseText.length - 4); // Supprimer '\n```'
    }
    if (responseText.endsWith('```')) {
      responseText = responseText.substring(0, responseText.length - 3); // Supprimer '```'
    }
...
IMPORTANT: Réponds UNIQUEMENT avec le JSON, sans markdown ni texte supplémentaire.`;
  ...
  async generateAdaptiveExercises(request: ExerciseRequest, count: number = 5): Promise<GeneratedExercise[]> {
    ...
    // Obtenir les compétences faibles pour ce module
    ...
    // Générer les exercices avec délai pour éviter les rate limits
    for (let i = 0; i < count; i++) {
      try {
        // Cibler une compétence faible spécifique
        ...
        // Délai entre les requêtes pour éviter les rate limits de Gemini
        if (i < count - 1) {
        ...
        // Utiliser un exercice de secours de la base de données
